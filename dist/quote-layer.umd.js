(function(h,$){typeof exports=="object"&&typeof module<"u"?$(exports):typeof define=="function"&&define.amd?define(["exports"],$):(h=typeof globalThis<"u"?globalThis:h||self,$(h.NymrelQuoteLayer=h.NymrelQuoteLayer||{}))})(this,(function(h){"use strict";var be=Object.defineProperty;var he=(h,$,g)=>$ in h?be(h,$,{enumerable:!0,configurable:!0,writable:!0,value:g}):h[$]=g;var w=(h,$,g)=>he(h,typeof $!="symbol"?$+"":$,g);function $(r="NYM"){const e=new Date,t=e.getFullYear(),i=String(e.getMonth()+1).padStart(2,"0"),n=String(e.getDate()).padStart(2,"0"),d=Math.random().toString(36).substring(2,8).toUpperCase();return`${r}-${t}${i}${n}-${d}`}function g(r,e="USD",t="$",i=0){if(isNaN(r)||r===null||r===void 0)return`${t}0`;const d=(i>0?r.toFixed(i):Math.round(r).toString()).split(".");return d[0]=d[0].replace(/\B(?=(\d{3})+(?!\d))/g,","),`${t}${d.join(".")}`}function C(r){return r==null?r:typeof r=="string"?r.replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi,"").replace(/<[^>]*>/g,"").replace(/[<>]/g,"").trim():typeof r=="number"?isNaN(r)?0:r:Array.isArray(r)?r.map(C):r}function q(r,e){if(!r||!r.fieldId)return!0;const t=e[r.fieldId];switch(r.operator){case"equals":return t===r.value||String(t)===String(r.value);case"notEquals":return t!==r.value&&String(t)!==String(r.value);case"greaterThan":return Number(t)>Number(r.value);case"lessThan":return Number(t)<Number(r.value);case"in":return Array.isArray(r.value)?r.value.includes(t):!1;case"contains":return Array.isArray(t)?t.includes(r.value):typeof t=="string"?t.includes(String(r.value)):!1;default:return!0}}function re(r,e){if(r.required&&(e==null||e===""||Array.isArray(e)&&e.length===0))return{valid:!1,error:`${r.label} is required.`};if(e!=null&&e!==""){if(r.type==="number"||r.type==="slider"||r.type==="stepper"){const t=Number(e);if(isNaN(t))return{valid:!1,error:`${r.label} must be a valid number.`};if(r.min!==void 0&&t<r.min)return{valid:!1,error:`Minimum value is ${r.min} ${r.unit||""}`.trim()};if(r.max!==void 0&&t>r.max)return{valid:!1,error:`Maximum value is ${r.max} ${r.unit||""}`.trim()}}if(r.type==="email"&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(String(e)))return{valid:!1,error:"Please enter a valid email address."};if(r.type==="phone"){const t=String(e).replace(/[\s()\-\.]/g,""),i=/^\+?[0-9]{7,15}$/;if(t.length<7||!i.test(t))return{valid:!1,error:"Please enter a valid phone number."}}}return{valid:!0}}function _(r,e){if(!e||e==="none")return r;switch(e){case"round":return Math.round(r);case"ceil":return Math.ceil(r);case"floor":return Math.floor(r);case"nearest10":return Math.round(r/10)*10;case"nearest50":return Math.round(r/50)*50;case"nearest100":return Math.round(r/100)*100;default:return r}}function P(r,e){var ee,te;const t=((ee=r.pricing)==null?void 0:ee.currency)||"USD",i=((te=r.pricing)==null?void 0:te.currencySymbol)||"$",n=r.pricing||{},d=e._quoteId||$(),p=[];if(r.steps.forEach(s=>{s.fields.forEach(k=>{p.push(k)})}),n.formula==="custom"&&typeof n.customFormula=="function"){const s=n.customFormula(e,p),k=_(s.target,n.rounding),b=_(s.min,n.rounding),u=_(s.max,n.rounding);return{min:b,max:u,target:k,formattedMin:g(b,t,i),formattedMax:g(u,t,i),formattedTarget:g(k,t,i),currency:t,currencySymbol:i,breakdown:s.breakdown||[],recommendations:[],calculatedAt:new Date().toISOString(),quoteId:d}}let m=Number(n.baseFee||n.baseCalloutFee||0),o=0,l=0,c=0,v=1;const y=[],x=[];m>0&&y.push({id:"base-fee",label:"Base Callout / Setup Fee",amount:m,formattedAmount:g(m,t,i),type:"base",description:"Standard initial mobilization and inspection base"});for(const s of p){if(!q(s.condition,e))continue;const k=e[s.id]!==void 0?e[s.id]:s.defaultValue;if(!(k==null||k==="")){if(s.type==="number"||s.type==="slider"||s.type==="stepper"){const b=Number(k);if(!isNaN(b)&&b>0){if(s.unitPrice&&s.unitPrice>0){const u=b*s.unitPrice;s.category==="material"?o+=u:l+=u,y.push({id:s.id,label:`${s.label} (${b} ${s.unit||"units"} @ ${g(s.unitPrice,t,i)}/${s.unit||"unit"})`,amount:u,formattedAmount:g(u,t,i),type:s.category==="material"?"material":"labor"})}s.multiplier&&s.multiplier!==1&&(v*=s.multiplier)}}if((s.type==="select"||s.type==="radio"||s.type==="toggle")&&s.options&&s.options.length>0){const b=s.options.find(u=>String(u.id)===String(k)||String(u.value)===String(k));if(b){if(b.adder&&Number(b.adder)!==0){const u=Number(b.adder);c+=u,y.push({id:`${s.id}-${b.id}`,label:`${s.label}: ${b.label}`,amount:u,formattedAmount:g(u,t,i),type:s.category==="material"?"material":"addon",description:b.description})}if(b.multiplier&&Number(b.multiplier)!==1){const u=Number(b.multiplier);v*=u,y.push({id:`${s.id}-${b.id}-mult`,label:`${b.label} Factor (${u}x)`,amount:0,formattedAmount:`${u}x`,type:"multiplier",description:b.description})}}}if(s.type==="checkbox")if(Array.isArray(k)&&s.options)for(const b of k){const u=s.options.find(F=>String(F.id)===String(b)||String(F.value)===String(b));if(u){if(u.adder&&Number(u.adder)!==0){const F=Number(u.adder);c+=F,y.push({id:`${s.id}-${u.id}`,label:u.label,amount:F,formattedAmount:g(F,t,i),type:"addon",description:u.description})}u.multiplier&&Number(u.multiplier)!==1&&(v*=Number(u.multiplier))}}else typeof k=="boolean"&&k===!0&&(s.unitPrice&&(c+=s.unitPrice,y.push({id:s.id,label:s.label,amount:s.unitPrice,formattedAmount:g(s.unitPrice,t,i),type:"addon"})),s.multiplier&&s.multiplier!==1&&(v*=s.multiplier))}}const f=m+o+l+c;let S=f*v;if(n.taxRate&&n.taxRate>0){const s=S*n.taxRate;y.push({id:"tax",label:`Estimated Tax (${(n.taxRate*100).toFixed(1)}%)`,amount:s,formattedAmount:g(s,t,i),type:"tax"}),S+=s}const E=n.marginPercent?n.marginPercent/100:.1,z=n.minRangeSpreadPercent?n.minRangeSpreadPercent/100:E,I=n.maxRangeSpreadPercent?n.maxRangeSpreadPercent/100:E;let L=Math.max(0,S),Q=Math.max(0,L*(1-z)),J=Math.max(Q,L*(1+I));f===0&&m===0&&(L=0,Q=0,J=0);const H=_(L,n.rounding),K=_(Q,n.rounding),X=_(J,n.rounding);return(e.pitch==="steep"||e.difficulty==="extreme"||e.urgency==="emergency")&&x.push("Priority Crew Dispatch: Includes safety rigging and on-site supervisor."),H>5e3&&x.push("Flexible Financing Available: 0% APR for 12 months on qualifying projects."),(e.material==="metal"||e.efficiency==="ultra")&&x.push("Qualifies for Energy Efficiency Tax Credits & Lifetime Manufacturer Warranty."),{min:K,max:X,target:H,formattedMin:g(K,t,i),formattedMax:g(X,t,i),formattedTarget:g(H,t,i),currency:t,currencySymbol:i,breakdown:y,recommendations:x,calculatedAt:new Date().toISOString(),quoteId:d}}function V(r=new Date().toISOString()){return{status:"not_configured",channel:"none",attemptedAt:r,message:"Local capture only: no delivery destination is configured, so nothing was sent."}}function ae(r=new Date().toISOString()){return{status:"callback_only",channel:"callback",attemptedAt:r,message:"Captured locally and handed to this page. No external delivery was attempted."}}async function ie(r,e){const t=new Date().toISOString();try{const i=await fetch(r,{method:"POST",headers:{"Content-Type":"application/json","X-Nymrel-Quote-Id":e.quoteId},body:JSON.stringify(e)});return i.ok?{status:"accepted",channel:"webhook",attemptedAt:t,httpStatus:i.status,ok:!0,message:"Your quote was accepted by the destination."}:{status:"rejected",channel:"webhook",attemptedAt:t,httpStatus:i.status,ok:!1,message:`Delivery failed: the destination declined this quote (HTTP ${i.status}). Your quote is shown below for your records.`}}catch{return{status:"failed",channel:"webhook",attemptedAt:t,ok:!1,message:"Delivery failed: could not reach the destination. Your quote is shown below for your records."}}}function ne(r){return r?r.message:V().message}function M(){return"sess_"+Math.random().toString(36).substring(2,10)+Date.now().toString(36)}function oe(){if(typeof window>"u")return"desktop";const r=navigator.userAgent.toLowerCase(),e=window.innerWidth||1024;return/(tablet|ipad|playbook|silk)|(android(?!.*mobi))/i.test(r)||e>=768&&e<=1024?"tablet":/Mobile|iP(hone|od)|Android|BlackBerry|IEMobile|Kindle|Silk-Accelerated|(hpw|web)OS|Opera M(obi|ini)/i.test(r)||e<768?"mobile":"desktop"}function se(r){if(!r)return"";try{return new URL(r).hostname}catch{return""}}function T(r={}){const e=typeof window<"u",t=e?new URLSearchParams(window.location.search):new URLSearchParams,i=e&&document.referrer||"",n=e&&window.location.href||"",d=e&&document.title||"",p=e&&navigator.userAgent||"";let m=r.customSessionId||"";if(!m&&e){const I=r.storageKey||"nymrel_quote_session_id";try{m=window.sessionStorage.getItem(I)||"",m||(m=M(),window.sessionStorage.setItem(I,m))}catch{m=M()}}else m||(m=M());const o=t.get("utm_source")||void 0,l=t.get("utm_medium")||void 0,c=t.get("utm_campaign")||void 0,v=t.get("utm_term")||void 0,y=t.get("utm_content")||void 0,x=t.get("gclid")||void 0,f=t.get("fbclid")||void 0,S=t.get("msclkid")||void 0,E=t.get("ttclid")||void 0,z=t.get("li_fat_id")||void 0;return{utm_source:o,utm_medium:l,utm_campaign:c,utm_term:v,utm_content:y,gclid:x,fbclid:f,msclkid:S,ttclid:E,li_fat_id:z,referrer:i,referring_domain:se(i),landing_page:n,page_title:d,timestamp:new Date().toISOString(),source_label:r.sourceLabel,session_id:m,device_type:oe(),userAgent:p}}function A(r,e={},t={}){if(typeof window>"u")return;const i=t.pushToDataLayer!==!1,n=t.useGtag!==!1,d=t.dispatchDomEvent!==!1,m=`${t.prefix||"nymrel_quote"}_${r}`,o={event:m,...e,timestamp:new Date().toISOString()};if(i){const l=window;l.dataLayer=l.dataLayer||[],l.dataLayer.push(o)}if(n){const l=window;typeof l.gtag=="function"&&l.gtag("event",m,e)}if(d)try{const l=new CustomEvent(m,{bubbles:!0,cancelable:!0,detail:o});window.dispatchEvent(l)}catch{}}function le(r,e){A("viewed",{schemaId:r,...e})}function de(r,e,t,i){A("step_completed",{schemaId:r,stepIndex:e,stepTitle:t,timeSpentMs:i})}function ce(r,e,t,i,n){A("calculated",{schemaId:r,target:e,min:t,max:i,quoteId:n})}function me(r,e,t,i){A("lead_submitted",{schemaId:r,quoteId:e,value:t,currency:"USD",hasEmail:!!i})}const j={mode:"warm",fontFamily:'-apple-system, BlinkMacSystemFont, "Plus Jakarta Sans", "Segoe UI", Roboto, "Helvetica Neue", sans-serif',primaryColor:"#2A332E",accentColor:"#A8541F",accentHoverColor:"#8E4316",backgroundColor:"#FAF8F2",surfaceColor:"#F4F0E6",cardColor:"#FFFFFF",textColor:"#2A332E",mutedTextColor:"#637069",borderColor:"#E2DCCE",borderRadius:"14px",boxShadow:"0 8px 30px -4px rgba(42, 51, 46, 0.08), 0 2px 8px -2px rgba(42, 51, 46, 0.04)",focusRingColor:"rgba(168, 84, 31, 0.28)"},W={mode:"light",fontFamily:'-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif',primaryColor:"#0F172A",accentColor:"#2563EB",accentHoverColor:"#1D4ED8",backgroundColor:"#F8FAFC",surfaceColor:"#F1F5F9",cardColor:"#FFFFFF",textColor:"#0F172A",mutedTextColor:"#64748B",borderColor:"#E2E8F0",borderRadius:"12px",boxShadow:"0 4px 20px -2px rgba(0, 0, 0, 0.06)",focusRingColor:"rgba(37, 99, 235, 0.25)"},U={mode:"dark",fontFamily:'-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif',primaryColor:"#F1F5F9",accentColor:"#F97316",accentHoverColor:"#EA580C",backgroundColor:"#0F172A",surfaceColor:"#1E293B",cardColor:"#1E293B",textColor:"#F8FAFC",mutedTextColor:"#94A3B8",borderColor:"#334155",borderRadius:"12px",boxShadow:"0 8px 30px -4px rgba(0, 0, 0, 0.4)",focusRingColor:"rgba(249, 115, 22, 0.3)"};function ue(r){return{...(r==null?void 0:r.mode)==="dark"?U:(r==null?void 0:r.mode)==="light"?W:j,...r}}function pe(r){return`
    ...
    --nym-success: #2E6B4F;
    --nym-error: #B83A2C;
  `}function O(r){const e=ue(r);return`
    :host {
      display: block;
  ...
  `}function a(r){return r==null?"":String(r).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}class R{constructor(e,t){w(this,"container");w(this,"schema");w(this,"callbacks");w(this,"formState",{});w(this,"currentStepIndex",0);w(this,"currentQuote");w(this,"showBreakdownModal",!1);w(this,"isSubmitted",!1);w(this,"isSubmitting",!1);w(this,"lastSubmission",null);w(this,"lastDeliveryReceipt",null);w(this,"sourceLabel");w(this,"webhookUrl");w(this,"fieldErrors",{});w(this,"stepStartTime",Date.now());this.schema=t.schema,this.callbacks=t.callbacks||{},this.sourceLabel=t.sourceLabel||this.schema.sourceLabel,this.webhookUrl=t.webhookUrl||this.schema.webhookUrl,this.schema.steps.forEach(i=>{i.fields.forEach(n=>{n.defaultValue!==void 0&&(this.formState[n.id]=n.defaultValue)})}),t.initialState&&(this.formState={...this.formState,...t.initialState}),t.useShadowDom!==!1&&e.attachShadow?e.shadowRoot?this.container=e.shadowRoot:this.container=e.attachShadow({mode:"open"}):this.container=e,this.currentQuote=P(this.schema,this.formState),le(this.schema.id,{sourceLabel:this.sourceLabel}),this.render()}updateState(e,t){this.formState[e]=C(t),delete this.fieldErrors[e],this.recalculate()}recalculate(){return this.currentQuote=P(this.schema,this.formState),ce(this.schema.id,this.currentQuote.target,this.currentQuote.min,this.currentQuote.max,this.currentQuote.quoteId),this.callbacks.onCalculate&&this.callbacks.onCalculate(this.currentQuote,this.formState),this.render(),this.currentQuote}nextStep(){if(!this.isLeadFormStep()){const t=this.schema.steps[this.currentStepIndex];let i=!1;this.fieldErrors={};for(const d of t.fields){if(!q(d.condition,this.formState))continue;const p=re(d,this.formState[d.id]);p.valid||(this.fieldErrors[d.id]=p.error||"Invalid value",i=!0)}if(i)return this.render(),!1;const n=Date.now()-this.stepStartTime;if(de(this.schema.id,this.currentStepIndex,t.title,n),this.currentStepIndex<this.schema.steps.length-1)return this.currentStepIndex++,this.stepStartTime=Date.now(),this.callbacks.onStepChange&&this.callbacks.onStepChange(this.currentStepIndex,this.schema.steps[this.currentStepIndex]),this.render(),!0;if(this.isLeadFormEnabled())return this.currentStepIndex++,this.stepStartTime=Date.now(),this.render(),!0}return!1}prevStep(){this.currentStepIndex>0&&(this.currentStepIndex--,this.stepStartTime=Date.now(),this.callbacks.onStepChange&&this.currentStepIndex<this.schema.steps.length&&this.callbacks.onStepChange(this.currentStepIndex,this.schema.steps[this.currentStepIndex]),this.render())}isLeadFormEnabled(){var e;return((e=this.schema.leadForm)==null?void 0:e.enabled)!==!1}isLeadFormStep(){return this.isLeadFormEnabled()&&this.currentStepIndex===this.schema.steps.length}getTotalStepsCount(){return this.schema.steps.length+(this.isLeadFormEnabled()?1:0)}async submitLead(e){var p,m,o;const t=this.schema.leadForm;this.fieldErrors={},(!e.name||String(e.name).trim()==="")&&(this.fieldErrors.lead_name="Full Name is required.");const i=/^[^\s@]+@[^\s@]+\.[^\s@]+$/;if((!e.email||!i.test(String(e.email)))&&(this.fieldErrors.lead_email="A valid email address is required."),t!=null&&t.requirePhone){const l=/^[+]?[(]?[0-9]{3}[)]?[-\s.]?[0-9]{3}[-\s.]?[0-9]{4,6}$/;(!e.phone||!l.test(String(e.phone)))&&(this.fieldErrors.lead_phone="Phone number is required.")}if(t!=null&&t.requireAddress&&!e.address&&(this.fieldErrors.lead_address="Street address or zip code is required."),Object.keys(this.fieldErrors).length>0)return this.render(),null;this.isSubmitting=!0,this.render();const n=T({sourceLabel:this.sourceLabel}),d={quoteId:this.currentQuote.quoteId,schemaId:this.schema.id,schemaName:this.schema.name,quote:this.currentQuote,formState:{...this.formState},lead:{name:C(e.name),email:C(e.email),phone:C(e.phone||""),address:C(e.address||""),zipCode:C(e.zipCode||""),preferredDate:C(e.preferredDate||""),preferredTime:C(e.preferredTime||""),notes:C(e.notes||"")},attribution:n,submittedAt:new Date().toISOString(),metadata:this.schema.metadata};try{if(this.webhookUrl){const l=await ie(this.webhookUrl,d);(l.status==="rejected"||l.status==="failed")&&console.warn("[NymrelQuote] Webhook delivery notice:",l.message),d.delivery=l}else d.delivery=this.callbacks.onSubmit?ae():V();if(this.callbacks.onSubmit)try{await this.callbacks.onSubmit(d)}catch(l){d.localHandlingFailed=!0,((p=d.delivery)==null?void 0:p.channel)==="callback"&&(d.delivery={...d.delivery,status:"failed",ok:!1,message:"The page handler failed. No external delivery was attempted by the widget. Your quote is shown below."});try{(o=(m=this.callbacks).onError)==null||o.call(m,l)}catch{}}try{me(this.schema.id,d.quoteId,this.currentQuote.target,d.lead.email)}catch{}return this.isSubmitting=!1,this.isSubmitted=!0,this.lastSubmission=d,this.lastDeliveryReceipt=d.delivery??null,this.render(),d}catch(l){return this.isSubmitting=!1,this.callbacks.onError&&this.callbacks.onError(l),this.fieldErrors._global="Submission failed. Please check your connection and try again.",this.render(),null}}getLastDeliveryReceipt(){return this.lastDeliveryReceipt}reset(){this.formState={},this.schema.steps.forEach(e=>{e.fields.forEach(t=>{t.defaultValue!==void 0&&(this.formState[t.id]=t.defaultValue)})}),this.currentStepIndex=0,this.isSubmitted=!1,this.isSubmitting=!1,this.lastSubmission=null,this.lastDeliveryReceipt=null,this.fieldErrors={},this.recalculate()}printReceipt(){typeof window<"u"&&window.print()}render(){var d;const e=O(this.schema.theme);let t="";this.isSubmitted&&this.lastSubmission?t=this.renderSuccessScreen(this.lastSubmission):this.isLeadFormStep()?t=this.renderLeadFormStep():t=this.renderStepForm(this.schema.steps[this.currentStepIndex]);const i=this.showBreakdownModal?this.renderBreakdownModal():"",n=`
      <style>${e}</style>
      <div class="nym-container" role="region" aria-label="${a(this.schema.name)}">
        ...
        <div class="nym-stepper" aria-label="Quote Progress">
          ${Array.from({length:this.getTotalStepsCount()}).map((p,m)=>{let o="nym-step-dot";return m===this.currentStepIndex&&(o+=" active"),m<this.currentStepIndex&&(o+=" completed"),`<div class="${o}"></div>`}).join("")}
        </div>
        <div class="nym-step-legend">
          <span>Step ${this.currentStepIndex+1} of ${this.getTotalStepsCount()}</span>
          <span>${this.isLeadFormStep()?"Lead Contact & Booking":a(((d=this.schema.steps[this.currentStepIndex])==null?void 0:d.title)||"")}</span>
        </div>
    ...
    `;this.container.innerHTML=n,this.attachEventListeners()}renderStepForm(e){const t=this.currentStepIndex===0;return`
    ...
    `}renderField(e){var d,p,m;if(!q(e.condition,this.formState))return"";const t=this.formState[e.id]!==void 0?this.formState[e.id]:e.defaultValue??"",i=this.fieldErrors[e.id];let n="";switch(e.type){case"slider":{const o=e.min??100,l=e.max??5e3,c=e.step??50,v=Number(t)||o;n=`
        ...
        `;break}case"select":{n=`
          <select class="nym-select nym-reactive-input" data-field-id="${e.id}" aria-label="${a(e.label)}">
            ${(d=e.options)==null?void 0:d.map(o=>`
              <option value="${a(o.id)}" ${String(t)===String(o.id)?"selected":""}>
                ${a(o.label)} ${o.adder?`(+${this.currentQuote.currencySymbol}${o.adder})`:""} ${o.multiplier?`(${o.multiplier}x)`:""}
              </option>
            `).join("")}
          </select>
        `;break}case"radio":{n=`
          <div class="nym-options-grid" role="radiogroup" aria-label="${a(e.label)}">
            ${(p=e.options)==null?void 0:p.map(o=>{const l=String(t)===String(o.id);return`
                <div 
                  class="nym-option-card ${l?"selected":""}" 
                  data-field-id="${e.id}" 
                  data-option-id="${a(o.id)}"
                  role="radio"
                  aria-checked="${l}"
                  tabindex="0"
                >
                  <div class="nym-option-header">
                    <span class="nym-option-title">${a(o.label)}</span>
                    ${o.badge?`<span class="nym-option-badge">${a(o.badge)}</span>`:""}
                  </div>
                  ${o.description?`<p class="nym-option-desc">${a(o.description)}</p>`:""}
        ...
        `;break}case"checkbox":{const o=Array.isArray(t)?t:[];n=`
          <div class="nym-checkbox-list">
            ${(m=e.options)==null?void 0:m.map(l=>{const c=o.includes(l.id);return`
                <div 
                  class="nym-checkbox-item ${c?"checked":""}" 
                  data-field-id="${e.id}" 
                  data-checkbox-id="${a(l.id)}"
                  role="checkbox"
                  aria-checked="${c}"
                  tabindex="0"
                >
                  ...
                  <div class="nym-checkbox-info">
                    ...
                    ${l.description?`<div class="nym-checkbox-desc">${a(l.description)}</div>`:""}
                  </div>
                </div>
              `}).join("")}
          </div>
        `;break}case"number":case"stepper":{n=`
          <input 
            type="number" 
      ...
      <div class="nym-field-group">
        ...
        ${i?`<div class="nym-error-text">${a(i)}</div>`:""}
      </div>
    ...
    `}renderSuccessScreen(e){var n;const t=this.schema.leadForm,i=((n=e.delivery)==null?void 0:n.status)==="accepted"&&!e.localHandlingFailed;return`
      <div class="nym-success-screen">
        <div class="nym-success-icon">✓</div>
        <h3 class="nym-step-title">${a(i&&(t==null?void 0:t.successTitle)||"Estimate Captured Successfully!")}</h3>
        <p class="nym-step-subtitle">${a(i&&(t==null?void 0:t.successMessage)||"Your quote summary is below. Print or save a copy for your records.")}</p>
        ...
        <div class="nym-receipt-card">
          <div class="nym-delivery-status nym-receipt-row" role="status">
            ...
            <span class="nym-receipt-val">${a(ne(this.lastDeliveryReceipt))}</span>
          </div>
