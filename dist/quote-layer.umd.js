  ...
  `}function C(e){return e==null?``:String(e).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`).replace(/'/g,`&#039;`)}var w=class{container;schema;callbacks;formState={};currentStepIndex=0;currentQuote;showBreakdownModal=!1;isSubmitted=!1;isSubmitting=!1;lastSubmission=null;sourceLabel;webhookUrl;fieldErrors={};stepStartTime=Date.now();constructor(e,t){this.schema=t.schema,this.callbacks=t.callbacks||{},this.sourceLabel=t.sourceLabel||this.schema.sourceLabel,this.webhookUrl=t.webhookUrl||this.schema.webhookUrl,this.schema.steps.forEach(e=>{e.fields.forEach(e=>{e.defaultValue!==void 0&&(this.formState[e.id]=e.defaultValue)})}),t.initialState&&(this.formState={...this.formState,...t.initialState}),this.container=t.useShadowDom!==!1&&e.attachShadow?e.shadowRoot?e.shadowRoot:e.attachShadow({mode:`open`}):e,this.currentQuote=s(this.schema,this.formState),p(this.schema.id,{sourceLabel:this.sourceLabel}),this.render()}updateState(e,t){this.formState[e]=r(t),delete this.fieldErrors[e],this.recalculate()}recalculate(){return this.currentQuote=s(this.schema,this.formState),h(this.schema.id,this.currentQuote.target,this.currentQuote.min,this.currentQuote.max,this.currentQuote.quoteId),this.callbacks.onCalculate&&this.callbacks.onCalculate(this.currentQuote,this.formState),this.render(),this.currentQuote}nextStep(){if(!this.isLeadFormStep()){let e=this.schema.steps[this.currentStepIndex],t=!1;this.fieldErrors={};for(let n of e.fields){if(!i(n.condition,this.formState))continue;let e=a(n,this.formState[n.id]);e.valid||(this.fieldErrors[n.id]=e.error||`Invalid value`,t=!0)}if(t)return this.render(),!1;let n=Date.now()-this.stepStartTime;if(m(this.schema.id,this.currentStepIndex,e.title,n),this.currentStepIndex<this.schema.steps.length-1)return this.currentStepIndex++,this.stepStartTime=Date.now(),this.callbacks.onStepChange&&this.callbacks.onStepChange(this.currentStepIndex,this.schema.steps[this.currentStepIndex]),this.render(),!0;if(this.isLeadFormEnabled())return this.currentStepIndex++,this.stepStartTime=Date.now(),this.render(),!0}return!1}prevStep(){this.currentStepIndex>0&&(this.currentStepIndex--,this.stepStartTime=Date.now(),this.callbacks.onStepChange&&this.currentStepIndex<this.schema.steps.length&&this.callbacks.onStepChange(this.currentStepIndex,this.schema.steps[this.currentStepIndex]),this.render())}isLeadFormEnabled(){return this.schema.leadForm?.enabled!==!1}isLeadFormStep(){return this.isLeadFormEnabled()&&this.currentStepIndex===this.schema.steps.length}getTotalStepsCount(){return this.schema.steps.length+ +!!this.isLeadFormEnabled()}async submitLead(e){let t=this.schema.leadForm;if(this.fieldErrors={},(!e.name||String(e.name).trim()===``)&&(this.fieldErrors.lead_name=`Full Name is required.`),(!e.email||!/^[^\\s@]+@[^\\s@]+\\.[^\\s@]+$/.test(String(e.email)))&&(this.fieldErrors.lead_email=`A valid email address is required.`),t?.requirePhone&&(!e.phone||!/^[+]?[(]?[0-9]{3}[)]?[-\\s.]?[0-9]{3}[-\\s.]?[0-9]{4,6}$/.test(String(e.phone)))&&(this.fieldErrors.lead_phone=`Phone number is required.`),t?.requireAddress&&!e.address&&(this.fieldErrors.lead_address=`Street address or zip code is required.`),Object.keys(this.fieldErrors).length>0)return this.render(),null;this.isSubmitting=!0,this.render();let n=d({sourceLabel:this.sourceLabel}),i={quoteId:this.currentQuote.quoteId,schemaId:this.schema.id,schemaName:this.schema.name,quote:this.currentQuote,formState:{...this.formState},lead:{name:r(e.name),email:r(e.email),phone:r(e.phone||``),address:r(e.address||``),zipCode:r(e.zipCode||``),preferredDate:r(e.preferredDate||``),preferredTime:r(e.preferredTime||``),notes:r(e.notes||``)},attribution:n,submittedAt:new Date().toISOString(),metadata:this.schema.metadata};try{if(this.webhookUrl)try{await fetch(this.webhookUrl,{method:`POST`,headers:{"Content-Type":`application/json`,"X-Nymrel-Quote-Id":i.quoteId},body:JSON.stringify(i)})}catch(e){console.warn(`[NymrelQuote] Webhook delivery notice:`,e)}return this.callbacks.onSubmit&&await this.callbacks.onSubmit(i),g(this.schema.id,i.quoteId,this.currentQuote.target,i.lead.email),this.isSubmitting=!1,this.isSubmitted=!0,this.lastSubmission=i,this.render(),i}catch(e){return this.isSubmitting=!1,this.callbacks.onError&&this.callbacks.onError(e),this.fieldErrors._global=`Submission failed. Please check your connection and try again.`,this.render(),null}}reset(){this.formState={},this.schema.steps.forEach(e=>{e.fields.forEach(e=>{e.defaultValue!==void 0&&(this.formState[e.id]=e.defaultValue)})}),this.currentStepIndex=0,this.isSubmitted=!1,this.isSubmitting=!1,this.lastSubmission=null,this.fieldErrors={},this.recalculate()}printReceipt(){typeof window<`u`&&window.print()}render(){let e=S(this.schema.theme),t=``;t=this.isSubmitted&&this.lastSubmission?this.renderSuccessScreen(this.lastSubmission):this.isLeadFormStep()?this.renderLeadFormStep():this.renderStepForm(this.schema.steps[this.currentStepIndex]);let n=this.showBreakdownModal?this.renderBreakdownModal():``,r=`
      ...
      <div class="nym-container" role="region" aria-label="${C(this.schema.name)}">
        ...
        <div class="nym-range-banner">
          <div class="nym-range-info">
            ...
            <div class="nym-range-values">
              <span class="nym-range-amount">${C(this.currentQuote.formattedMin)}</span>
              <span class="nym-range-separator">&ndash;</span>
              <span class="nym-range-amount">${C(this.currentQuote.formattedMax)}</span>
            </div>
            <span class="nym-range-target">Baseline Target: <strong>${C(this.currentQuote.formattedTarget)}</strong></span>
          </div>
        ...
        <div class="nym-step-legend">
          ...
          <span>${this.isLeadFormStep()?`Lead Contact & Booking`:C(this.schema.steps[this.currentStepIndex]?.title||``)}</span>
        </div>
    ...
    `;this.container.innerHTML=r,this.attachEventListeners()}renderStepForm(e){let t=this.currentStepIndex===0;return`
      <div class="nym-step-view">
        <h3 class="nym-step-title">${C(e.title)}</h3>
        ${e.subtitle?`<p class="nym-step-subtitle">${C(e.subtitle)}</p>`:``}
    ...
    `}renderField(e){if(!i(e.condition,this.formState))return``;let t=this.formState[e.id]===void 0?e.defaultValue??``:this.formState[e.id],n=this.fieldErrors[e.id],r=``;switch(e.type){case`slider`:{let n=e.min??100,i=e.max??5e3,a=e.step??50,o=Number(t)||n;r=`
        ...
        `;break}case`select`:r=`
          <select class="nym-select nym-reactive-input" data-field-id="${e.id}" aria-label="${C(e.label)}">
            ${e.options?.map(e=>`
              <option value="${C(e.id)}" ${String(t)===String(e.id)?`selected`:``}>
                ${C(e.label)} ${e.adder?`(+${this.currentQuote.currencySymbol}${e.adder})`:``} ${e.multiplier?`(${e.multiplier}x)`:``}
              </option>
            `).join(``)}
          </select>
        `;break;case`radio`:r=`
          <div class="nym-options-grid" role="radiogroup" aria-label="${C(e.label)}">
            ${e.options?.map(n=>{let r=String(t)===String(n.id);return`
                <div
                  class="nym-option-card ${r?`selected`:``}"
                  data-field-id="${e.id}"
                  data-option-id="${C(n.id)}"
                  role="radio"
                  aria-checked="${r}"
                  tabindex="0"
                >
                  <div class="nym-option-header">
                    <span class="nym-option-title">${C(n.label)}</span>
                    ${n.badge?`<span class="nym-option-badge">${C(n.badge)}</span>`:``}
                  </div>
                  ${n.description?`<p class="nym-option-desc">${C(n.description)}</p>`:``}
            ...
            ${e.options?.map(t=>{let r=n.includes(t.id);return`
                ...
                >
                  ...
                  <div class="nym-checkbox-info">
                    ...
                    ${t.description?`<div class="nym-checkbox-desc">${C(t.description)}</div>`:``}
                  </div>
                </div>
              `}).join(``)}
          </div>
        `;break}case`number`:case`stepper`:r=`
          <input
            ...
            step="${e.step??1}"
            value="${C(t)}"
            placeholder="${e.placeholder?C(e.placeholder):``}"
            aria-label="${C(e.label)}"
          />
        `;break;default:r=`
          <input
      ...
      <div class="nym-field-group">
        <label class="nym-label">
          <span>${C(e.label)}${e.required?` *`:``}</span>
          ${e.unit&&e.type!==`slider`?`<span class="nym-helper-text">${C(e.unit)}</span>`:``}
        </label>
        ${r}
        ${e.helperText?`<div class="nym-helper-text">${C(e.helperText)}</div>`:``}
        ${n?`<div class="nym-error-text">${C(n)}</div>`:``}
      </div>
    `}renderLeadFormStep(){let e=this.schema.leadForm,t=this.fieldErrors._global;return`
      <div class="nym-lead-step">
        <h3 class="nym-step-title">${C(e?.title||`Lock in Your Official Quote`)}</h3>
        <p class="nym-step-subtitle">${C(e?.subtitle||`Enter your contact details to save your estimate, receive your official PDF breakdown, and schedule an on-site inspection.`)}</p>

        ${t?`<div class="nym-error-text" style="margin-bottom: 16px;">${C(t)}</div>`:``}

        <form id="nym-lead-form">
          <div class="nym-field-group">
            <label class="nym-label">Full Name *</label>
            <input type="text" name="name" class="nym-input" required placeholder="e.g. Alex Morgan" value="${C(this.formState._lead_name||``)}" />
            ${this.fieldErrors.lead_name?`<div class="nym-error-text">${C(this.fieldErrors.lead_name)}</div>`:``}
          </div>

          <div class="nym-field-group">
            <label class="nym-label">Email Address *</label>
            <input type="email" name="email" class="nym-input" required placeholder="alex@example.com" value="${C(this.formState._lead_email||``)}" />
            ${this.fieldErrors.lead_email?`<div class="nym-error-text">${C(this.fieldErrors.lead_email)}</div>`:``}
          </div>

          <div class="nym-field-group">
            <label class="nym-label">Phone Number ${e?.requirePhone?`*`:`(Optional)`}</label>
            <input type="tel" name="phone" class="nym-input" ${e?.requirePhone?`required`:``} placeholder="(555) 019-2834" value="${C(this.formState._lead_phone||``)}" />
            ${this.fieldErrors.lead_phone?`<div class="nym-error-text">${C(this.fieldErrors.lead_phone)}</div>`:``}
          </div>

          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
            <div class="nym-field-group">
              <label class="nym-label">Street Address ${e?.requireAddress?`*`:``}</label>
              <input type="text" name="address" class="nym-input" placeholder="123 Maple Way" value="${C(this.formState._lead_address||``)}" />
              ${this.fieldErrors.lead_address?`<div class="nym-error-text">${C(this.fieldErrors.lead_address)}</div>`:``}
            </div>
            <div class="nym-field-group">
              <label class="nym-label">Zip Code</label>
              <input type="text" name="zipCode" class="nym-input" placeholder="90210" value="${C(this.formState._lead_zip||``)}" />
            </div>
          </div>
          ...
          ${e?.requireDate?`
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
              <div class="nym-field-group">
                ...
                <input type="date" name="preferredDate" class="nym-input" value="${C(this.formState._lead_date||``)}" />
              </div>
              <div class="nym-field-group">
          ...
          ${e?.collectNotes===!1?``:`
            <div class="nym-field-group">
              ...
              <textarea name="notes" class="nym-textarea" placeholder="Tell us about specific property access, timeline goals, or special requirements...">${C(this.formState._lead_notes||``)}</textarea>
            </div>
          `}

          <div class="nym-helper-text" style="margin-bottom: 20px;">
            ${C(e?.disclaimerText||`By submitting, you agree to receive project updates and quote confirmation. We respect your privacy and never sell data.`)}
          </div>
    ...
    `}renderSuccessScreen(e){let t=this.schema.leadForm;return`
      <div class="nym-success-screen">
        ...
        <h3 class="nym-step-title">${C(t?.successTitle||`Estimate Successfully Saved & Confirmed!`)}</h3>
        <p class="nym-step-subtitle">${C(t?.successMessage||`A detailed quote confirmation and project summary have been dispatched to your email.`)}</p>

        <div class="nym-receipt-card">
          <div class="nym-receipt-row">
            <span class="nym-receipt-key">Quote Reference ID:</span>
            <span class="nym-receipt-val" style="font-family: monospace;">${C(e.quoteId)}</span>
          </div>
          <div class="nym-receipt-row">
            ...
            <span class="nym-receipt-val" style="color: var(--nym-accent); font-size: 1.05rem;">
              ${C(e.quote.formattedMin)} &ndash; ${C(e.quote.formattedMax)}
            </span>
          </div>
          <div class="nym-receipt-row">
            <span class="nym-receipt-key">Baseline Target:</span>
            <span class="nym-receipt-val">${C(e.quote.formattedTarget)}</span>
          </div>
          <div class="nym-receipt-row">
            <span class="nym-receipt-key">Recipient:</span>
            <span class="nym-receipt-val">${C(e.lead.name)} (${C(e.lead.email)})</span>
          </div>
          ${e.lead.preferredDate?`
            <div class="nym-receipt-row">
              <span class="nym-receipt-key">Requested Consultation:</span>
              <span class="nym-receipt-val">${C(e.lead.preferredDate)} (${C(e.lead.preferredTime||`Anytime`)})</span>
            </div>
          `:``}
        </div>
            ...
            <table class="nym-breakdown-table">
              <tbody>
                ${this.currentQuote.breakdown.map(e=>`
                  <tr>
                    <td>
                      <div class="nym-breakdown-label">${C(e.label)}</div>
                      ${e.description?`<div class="nym-breakdown-subtext">${C(e.description)}</div>`:``}
                    </td>
                    <td class="nym-breakdown-val">${C(e.formattedAmount)}</td>
                  </tr>
                `).join(``)}
                <tr class="nym-breakdown-total">
                  <td><strong>Estimated Baseline Target</strong></td>
                  <td class="nym-breakdown-val">${C(this.currentQuote.formattedTarget)}</td>
                </tr>
                <tr>
                  <td><strong>Dynamic Estimated Range</strong></td>
                  <td class="nym-breakdown-val">${C(this.currentQuote.formattedMin)} &ndash; ${C(this.currentQuote.formattedMax)}</td>
                </tr>
              </tbody>
            </table>
          </div>
