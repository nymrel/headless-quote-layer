...
function o(e, t = "USD", n = "$", r = 0) {
	if (isNaN(e) || e == null) return `${n}0`;
	let i = (r > 0 ? e.toFixed(r) : Math.round(e).toString()).split(".");
	return i[0] = i[0].replace(/\B(?=(\d{3})+(?!\d))/g, ","), `${n}${i.join(".")}`;
}
function s(e) {
	return e == null ? e : typeof e == "string" ? e.replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, "").replace(/<[^>]*>/g, "").replace(/[<>]/g, "").trim() : typeof e == "number" ? isNaN(e) ? 0 : e : Array.isArray(e) ? e.map(s) : e;
}
function c(e, t) {
...
function l(e, t) {
	...
	if (t != null && t !== "") {
		...
		if (e.type === "email" && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(String(t))) return {
			valid: !1,
...
function d(e, t) {
	let n = e.pricing?.currency || "USD", r = e.pricing?.currencySymbol || "$", i = e.pricing || {}, s = t._quoteId || a(), l = [];
	if (e.steps.forEach((e) => {
		e.fields.forEach((e) => {
			l.push(e);
		});
	}), i.formula === "custom" && typeof i.customFormula == "function") {
		let e = i.customFormula(t, l), a = u(e.target, i.rounding), c = u(e.min, i.rounding), d = u(e.max, i.rounding);
		return {
			min: c,
			max: d,
			target: a,
			formattedMin: o(c, n, r),
			formattedMax: o(d, n, r),
			formattedTarget: o(a, n, r),
			currency: n,
			currencySymbol: r,
	...
	let d = Number(i.baseFee || i.baseCalloutFee || 0), f = 0, p = 0, m = 0, h = 1, g = [], _ = [];
	d > 0 && g.push({
		id: "base-fee",
		label: "Base Callout / Setup Fee",
		amount: d,
		formattedAmount: o(d, n, r),
		type: "base",
		description: "Standard initial mobilization and inspection base"
	});
	for (let e of l) {
		if (!c(e.condition, t)) continue;
		let i = t[e.id] === void 0 ? e.defaultValue : t[e.id];
		if (i != null && i !== "") {
			if (e.type === "number" || e.type === "slider" || e.type === "stepper") {
				let t = Number(i);
				if (!isNaN(t) && t > 0) {
					if (e.unitPrice && e.unitPrice > 0) {
						let i = t * e.unitPrice;
						e.category === "material" ? f += i : p += i, g.push({
							id: e.id,
							label: `${e.label} (${t} ${e.unit || "units"} @ ${o(e.unitPrice, n, r)}/${e.unit || "unit"})`,
							amount: i,
							formattedAmount: o(i, n, r),
			...
			if ((e.type === "select" || e.type === "radio" || e.type === "toggle") && e.options && e.options.length > 0) {
				let t = e.options.find((e) => String(e.id) === String(i) || String(e.value) === String(i));
				if (t) {
					if (t.adder && Number(t.adder) !== 0) {
						let i = Number(t.adder);
						m += i, g.push({
							id: `${e.id}-${t.id}`,
					...
					if (t.multiplier && Number(t.multiplier) !== 1) {
						let n = Number(t.multiplier);
						h *= n, g.push({
							id: `${e.id}-${t.id}-mult`,
							label: `${t.label} Factor (${n}x)`,
							amount: 0,
							formattedAmount: `${n}x`,
							type: "multiplier",
							description: t.description
			...
			if (e.type === "checkbox") {
				if (Array.isArray(i) && e.options) for (let t of i) {
					let i = e.options.find((e) => String(e.id) === String(t) || String(e.value) === String(t));
					if (i) {
						if (i.adder && Number(i.adder) !== 0) {
							let t = Number(i.adder);
							m += t, g.push({
								id: `${e.id}-${i.id}`,
								label: i.label,
								amount: t,
								formattedAmount: o(t, n, r),
								type: "addon",
								description: i.description
							});
						}
						i.multiplier && Number(i.multiplier) !== 1 && (h *= Number(i.multiplier));
					}
				}
				else typeof i == "boolean" && i === !0 && (e.unitPrice && (m += e.unitPrice, g.push({
					id: e.id,
					label: e.label,
					amount: e.unitPrice,
					formattedAmount: o(e.unitPrice, n, r),
					type: "addon"
				})), e.multiplier && e.multiplier !== 1 && (h *= e.multiplier));
			}
		}
	...
	let v = d + f + p + m, y = v * h;
	if (i.taxRate && i.taxRate > 0) {
		let e = y * i.taxRate;
		g.push({
			id: "tax",
			label: `Estimated Tax (${(i.taxRate * 100).toFixed(1)}%)`,
			amount: e,
			formattedAmount: o(e, n, r),
			type: "tax"
		}), y += e;
	}
	let b = i.marginPercent ? i.marginPercent / 100 : .1, x = i.minRangeSpreadPercent ? i.minRangeSpreadPercent / 100 : b, S = i.maxRangeSpreadPercent ? i.maxRangeSpreadPercent / 100 : b, C = Math.max(0, y), w = Math.max(0, C * (1 - x)), T = Math.max(w, C * (1 + S));
	v === 0 && d === 0 && (C = 0, w = 0, T = 0);
	let E = u(C, i.rounding), D = u(w, i.rounding), O = u(T, i.rounding);
	return (t.pitch === "steep" || t.difficulty === "extreme" || t.urgency === "emergency") && _.push("Priority Crew Dispatch: Includes safety rigging and on-site supervisor."), E > 5e3 && _.push("Flexible Financing Available: 0% APR for 12 months on qualifying projects."), (t.material === "metal" || t.efficiency === "ultra") && _.push("Qualifies for Energy Efficiency Tax Credits & Lifetime Manufacturer Warranty."), {
		min: D,
		max: O,
		target: E,
		formattedMin: o(D, n, r),
		formattedMax: o(O, n, r),
		formattedTarget: o(E, n, r),
		currency: n,
		currencySymbol: r,
		breakdown: g,
		recommendations: _,
...
function h(e = {}) {
	...
	if (!s && t) {
		...
		try {
			s = window.sessionStorage.getItem(t) || "", s || (s = f(), window.sessionStorage.setItem(t, s));
		} catch {
			s = f();
		}
	} else s ||= f();
...
function T(e) {
	return {
		...e?.mode === "dark" ? w : e?.mode === "light" ? C : S,
		...e
	};
}
function E(e) {
...
var k = class {
	...
	constructor(e, t) {
		this.schema = t.schema, this.callbacks = t.callbacks || {}, this.sourceLabel = t.sourceLabel || this.schema.sourceLabel, this.webhookUrl = t.webhookUrl || this.schema.webhookUrl, this.schema.steps.forEach((e) => {
			...
			...this.formState,
			...t.initialState
		}), this.container = t.useShadowDom !== !1 && e.attachShadow ? e.shadowRoot ? e.shadowRoot : e.attachShadow({ mode: "open" }) : e, this.currentQuote = d(this.schema, this.formState), _(this.schema.id, { sourceLabel: this.sourceLabel }), this.render();
	}
	updateState(e, t) {
		this.formState[e] = s(t), delete this.fieldErrors[e], this.recalculate();
	}
	recalculate() {
		return this.currentQuote = d(this.schema, this.formState), y(this.schema.id, this.currentQuote.target, this.currentQuote.min, this.currentQuote.max, this.currentQuote.quoteId), this.callbacks.onCalculate && this.callbacks.onCalculate(this.currentQuote, this.formState), this.render(), this.currentQuote;
	}
	nextStep() {
		if (!this.isLeadFormStep()) {
			...
			for (let n of e.fields) {
				if (!c(n.condition, this.formState)) continue;
				let e = l(n, this.formState[n.id]);
				e.valid || (this.fieldErrors[n.id] = e.error || "Invalid value", t = !0);
			}
			if (t) return this.render(), !1;
			let n = Date.now() - this.stepStartTime;
			if (v(this.schema.id, this.currentStepIndex, e.title, n), this.currentStepIndex < this.schema.steps.length - 1) return this.currentStepIndex++, this.stepStartTime = Date.now(), this.callbacks.onStepChange && this.callbacks.onStepChange(this.currentStepIndex, this.schema.steps[this.currentStepIndex]), this.render(), !0;
	...
	async submitLead(e) {
		...
		let n = h({ sourceLabel: this.sourceLabel }), r = {
			...
			lead: {
				...
				address: s(e.address || ""),
				zipCode: s(e.zipCode || ""),
				preferredDate: s(e.preferredDate || ""),
				preferredTime: s(e.preferredTime || ""),
				notes: s(e.notes || "")
			},
			attribution: n,
		...
		try {
			if (this.webhookUrl) try {
				await fetch(this.webhookUrl, {
					method: "POST",
					headers: {
						"Content-Type": "application/json",
						"X-Nymrel-Quote-Id": r.quoteId
					},
					body: JSON.stringify(r)
				});
			} catch (e) {
				console.warn("[NymrelQuote] Webhook delivery notice:", e);
			}
			return this.callbacks.onSubmit && await this.callbacks.onSubmit(r), b(this.schema.id, r.quoteId, this.currentQuote.target, r.lead.email), this.isSubmitting = !1, this.isSubmitted = !0, this.lastSubmission = r, this.render(), r;
		} catch (e) {
	...
	renderField(e) {
		if (!c(e.condition, this.formState)) return "";
	...
	renderSuccessScreen(e) {
      ...
      <div class="nym-success-screen">
        ...
        <h3 class="nym-step-title">${O(t?.successTitle || "Estimate Successfully Saved & Confirmed!")}</h3>
        <p class="nym-step-subtitle">${O(t?.successMessage || "A detailed quote confirmation and project summary have been dispatched to your email.")}</p>
...
var j = ({ schema: e, initialState: r, theme: a, sourceLabel: o, webhookUrl: s, useShadowDom: c = !0, onCalculate: l, onStepChange: u, onSubmit: d, onError: f, className: p, style: m }) => {
	let h = n(null), g = n(null);
	return t(() => {
		if (!h.current) return;
		let t = a ? {
			...e,
		...
		return g.current = n, () => {
			g.current = null, h.current && (h.current.innerHTML = "");
		};
	}, [
		e,
		a,
		...
		ref: h,
		className: `nymrel-quote-widget-root ${p || ""}`.trim(),
		style: m
	});
};
function M(n, i) {
	let [a, o] = r(() => {
		let e = {};
		return n.steps.forEach((t) => {
			t.fields.forEach((t) => {
				t.defaultValue !== void 0 && (e[t.id] = t.defaultValue);
			});
		}), {
			...e,
			...i
		};
	}), [c, u] = r(0), [f, p] = r(() => d(n, a)), [m, g] = r({}), [_, v] = r(!1), [y, x] = r(!1), [S, C] = r(null);
	return t(() => {
		let e = d(n, a);
		p(e);
	}, [n, a]), {
		...
		nextStep: e(() => {
			...
			for (let n of e.fields) {
				let e = l(n, a[n.id]);
				e.valid || (r[n.id] = e.error || "Invalid value", t = !0);
			}
			return t ? (g(r), !1) : c < n.steps.length && (u((e) => e + 1), !0);
		}, [
			n,
			c,
			a
		...
		submitLead: e(async (e, t = {}) => {
			v(!0);
			let r = h({ sourceLabel: t.sourceLabel }), i = {
				quoteId: f.quoteId,
				schemaId: n.id,
				schemaName: n.name,
				quote: f,
			...
			if (t.webhookUrl) try {
				...
				console.warn("[useQuoteEngine] Webhook error:", e);
			}
			return b(n.id, i.quoteId, f.target, i.lead.email), v(!1), x(!0), C(i), i;
		}, [
			n,
			f,
			a
		...
		reset: e(() => {
			...
			n.steps.forEach((t) => {
				...
				...e,
				...i
			}), u(0), x(!1), v(!1), C(null), g({});
		}, [n, i])
	};
}
//#endregion
export { o as C, l as E, c as S, s as T, y as _, w as a, u as b, E as c, g as d, h as f, b as g, x as h, A as i, D as l, m, M as n, S as o, p, k as r, C as s, j as t, T as u, _ as v, a as w, d as x, v as y };
